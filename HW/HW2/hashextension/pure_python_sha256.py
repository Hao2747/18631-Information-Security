'''
This program is used to add contents behind original plaintext and make its hash still valid.
Note: the program should be run with python3.
Date:10/06/2021
...
def sha256(msg):
    ...
    # # Initialize the hash state to the hash value after calculating the hash of orginal plaintext
    h0 = 0x0189f6fa
    h1 = 0x69acb09e
    h2 = 0x366d3239
    h3 = 0xe20ea9c0
...
if __name__ == "__main__":
    ...
    # Concatenate a new cookie that uses original plaintext but manually padded 
    # so that the first 128 bytes will generate the same hash value as given in the CTF shell
    # new_cookie will be passed to CTF shell as plaintext message
    new_cookie = msg.hex() + "8" + "0" * 136 + "1D0" + b"admins".hex()
    
    # The extra part of message that we want to add after original plaintext. 
    # We manually pass this text to our hash function that start with 
    # modified state coming from the hash given in the shell
    extend_msg = b"admins".hex() + "8" + "0" * 112 + "430"
    print("new cookie {} with length {}".format(new_cookie, len(new_cookie)))
    print("extended msg {} with length {}".format(extend_msg, len(extend_msg)))
    sha256(extend_msg)
